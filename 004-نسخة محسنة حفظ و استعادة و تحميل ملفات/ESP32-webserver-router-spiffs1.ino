  ...
  <div class="card">
   ...
   <div class="collapsible-content" id="advancedSettings">
      ...
      <div class="preset-buttons">
        ...
        <button onclick="toggleUploadSection()" class="button button-upload"><i class="fas fa-cloud-upload-alt"></i> رفع ملف</button>   <!-- زر الرفع الجديد -->
      </div>
              <!-- قسم تحميل الملفات (مخفي افتراضيًا) -->
...
// ---- واجهة لرفع الملفات الى االشريحة --------
   const char* uploadHTML = R"rawliteral(
<div class="danger-zone">
  <h3>⚡ تحميل الملفات</h3>
     <!-- في قسم التحميل -->
<small>يجب أن تتبع التسمية: 
  <br>
  CSS: /css/اسم الملف.css
  <br>
  JS: /js/اسم الملف.js
  <br>
  Fonts: /fonts/اسم الملف.woff2
</small>
  <form id="uploadForm" enctype="multipart/form-data">
    <input type="file" name="file" id="fileInput" hidden>
    <label for="fileInput" class="button button-upload"><i class="fas fa-cloud-upload-alt"></i> اختر ملف</label>
     <button type="button" onclick="uploadFile()" class="button button-on"><i class="fas fa-upload"></i> رفع</button>
...
void handleFileUpload() {
  ...
  String targetDir = "/"; // المسار الافتراضي
  String filename;

  if (upload.status == UPLOAD_FILE_START) {
    // تحديد المجلد بناءً على نوع الملف
    if (upload.filename.endsWith(".css")) targetDir = "/css/";
    else if (upload.filename.endsWith(".js")) targetDir = "/js/";
    else if (upload.filename.endsWith(".woff2")) targetDir = "/fonts/";

    // إنشاء المسار الكامل
    String filename = targetDir + upload.filename;
    ...
    if (!filename.startsWith("/css/") && 
      ...
      // منع الوصول إلى الملفات النظامية
    if (filename.startsWith("/system/") || filename.equals("/wifi_config.txt")) {
      server.send(403, "text/plain", "ملف نظام - ممنوع التعديل");
      return;
    }
    if (server.client().remoteIP() != WiFi.localIP()) {
    server.send(403, "text/plain", "ممنوع الوصول");
    return;
  }

    // التحقق من المساحة
    if (SPIFFS.totalBytes() - SPIFFS.usedBytes() < upload.totalSize) {
      server.send(507, "text/plain", "مساحة غير كافية");
      return;
    }
      // تحديد الحد الأقصى لحجم الملف لكل نوع ---- مثال هنا ملف الستايل
 if (filename.endsWith(".css") && upload.totalSize > 20480) {
  server.send(413, "text/plain", "حجم ملف CSS يتجاوز 20KB");
  return;
 }
    ...
    if (!fsUploadFile) {
      server.send(500, "text/plain", "فشل إنشاء الملف");
    }
    ...
    if (fsUploadFile) {
      fsUploadFile.close();
      Serial.printf("تم التحميل: %s (%u bytes)\n", 
...
void setupServer() {
  ...
  server.on("/", HTTP_GET, []() {
    if (isConnected) {
      ...
      html.replace("%UPLOAD_FORM%", uploadHTML);
