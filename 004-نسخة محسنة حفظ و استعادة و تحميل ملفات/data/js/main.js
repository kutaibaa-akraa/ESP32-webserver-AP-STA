...
/*     for (let i = 1; i <= 10; i++) {
  ...
  uploadSection.style.display = 'none'; // إخفاء قسم الرفع عند فتح/غلق الإعدادات
  
  btnText.innerHTML = content.classList.contains('active') ? 
    'إخفاء الإعدادات ▲' : 'إظهار الإعدادات ▼';
}
...
function uploadFile() {
  ...
  if (!allowedPaths[file.type]) {
    alert('❌ نوع الملف غير مدعوم!');
    return;
  }
  
  if (user.isAdmin) showUploadButton();

  // إضافة المسار التصنيفي لاسم الملف
  const newName = allowedPaths[file.type] + file.name;
  ...
  .then(data => {
    document.getElementById('uploadStatus').innerHTML = 
      `<div class="upload-success">✓ ${data}</div>`;
