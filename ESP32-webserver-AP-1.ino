...
#define EEPROM_SIZE 64  // حجم EEPROM المستخدم
...
struct GpioPin {
  ...
  char name[32];                  // اسم العرض (تم تغييره إلى char[] بدلاً من const char*)
...
// ============ التصريح المسبق عن الدوال قبل دالة الإعداد setup() ============

// تُرجع حالة النظام الحالية على شكل JSON (عدد المخرجات، حالاتهم، الشبكة... إلخ)
String getSystemStatusJSON();

// تبديل حالة مخرج بناءً على رقم ترتيبه في المصفوفة pins[]
void toggleOutput(int pinIndex);

// تبديل جميع المخرجات التبادلية (تشغيل أو إيقاف جماعي)
void toggleOutputs();

// تبديل حالة مخرج معين بناءً على الرابط (path) المستدعى
void toggleOutput(const char* path);

// بدء تشغيل النظام التبادلي تلقائياً (مثل نمط متسلسل)
void startToggleSystem();

// إيقاف النظام التبادلي بالكامل
void stopToggleSystem();

// إيقاف مؤقت مؤقت للنظام التبادلي (pause فقط)
void pauseToggleSystem();

// استئناف التشغيل التبادلي بعد الإيقاف المؤقت
void resumeToggleSystem();

// حساب الوقت المتبقي لإيقاف تتابع تلقائي
unsigned long calculateRemainingTime();

// حساب نسبة التقدم في التتابع (كـ %)
int calculateProgress();

// محاولة الاتصال بشبكة WiFi باستخدام إعدادات مخزنة أو مبرمجة
void connectToWiFi();

// بدء نقطة وصول (Access Point) في حال فشل الاتصال بالواي فاي
void startAPMode();

// تهيئة خادم الويب ومسارات الطلبات
void setupServer();

// توليد صفحة إعدادات شبكة WiFi أو الإعدادات العامة (HTML)
void handleConfigPage();

// حفظ إعدادات المستخدم المرسلة من صفحة الإعدادات (مثل WiFi أو مؤقت)
void handleSaveConfig();

// عرض الصفحة الرئيسية التي تحتوي على أزرار التحكم بالمخارج
void handleRoot();

// حفظ حالة جميع المخارج إلى EEPROM
void savePinStatesToEEPROM();

// تهيئة المخارج وإعادة تحميل حالتها من EEPROM
void setupPins();

// توليد صفحة إعدادات تخصيص أسماء المخارج ومؤقتاتها
void handleSettings();

// حفظ إعدادات الأسماء والمؤقتات من صفحة الإعدادات
void handleSaveSettings();

// حفظ إعدادات الاتصال بالشبكة WiFi إلى EEPROM
void saveWiFiConfig();
...
void setup() {
  ...
  for (const GpioPin& pin : pins) {
    server.on(pin.onCmd, [pin]() mutable { toggleOutput(pin.onCmd); });
    server.on(pin.offCmd, [pin]() mutable { toggleOutput(pin.offCmd); });
  }
...
void loop() {
  ...
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    if (pins[i].autoOffDuration > 0 && pins[i].state == 1 && now - pins[i].activationTime >= pins[i].autoOffDuration) {
      digitalWrite(pins[i].number, LOW);
      pins[i].state = 0;
      savePinStatesToEEPROM();
    }
  }
 ...
 // إعادة الاتصال التلقائي إذا فُقد الاتصال
  ...
  if (millis() - lastCheck >= 1000) {
    ...
    for (int i = 2; i <= 11; i++) {
      ...
      // تحقق من الإيقاف التلقائي إن وُجد
  unsigned long now = millis();
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    if (pins[i].autoOffDuration > 0 && pins[i].state == 1 && now - pins[i].activationTime >= pins[i].autoOffDuration) {
      digitalWrite(pins[i].number, LOW);
...
void setupPins() {
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    ...
    pins[i].state = EEPROM.read(i);
    EEPROM.get(20 + i * sizeof(unsigned long), pins[i].autoOffDuration);
    EEPROM.get(100 + i * 32, pins[i].name);
    digitalWrite(pins[i].number, pins[i].state);
  }
}

void handleRoot() {
  String html = "<html><head><meta charset='UTF-8'><title>ESP32 Web Control</title></head><body><h2>تحكم في المخارج</h2>";
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    html += pins[i].name;
    html += " - الحالة: ";
    html += (pins[i].state ? "تشغيل" : "إيقاف");
    if (pins[i].allowManualControl) {
      html += " <a href='" + String(pins[i].onCmd) + "'>تشغيل</a> | ";
      html += "<a href='" + String(pins[i].offCmd) + "'>إيقاف</a><br>";
    } else {
      html += "<br>";
    }
  }
  html += "<br><a href='/settings'>إعدادات المخارج</a>";
  html += "</body></html>";
  server.send(200, "text/html", html);
}
...
void savePinStatesToEEPROM() {
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    EEPROM.write(i, pins[i].state);
    EEPROM.put(20 + i * sizeof(unsigned long), pins[i].autoOffDuration);
    EEPROM.put(100 + i * 32, pins[i].name);
  }
  EEPROM.commit();
}
...
void toggleOutput(const char* path) {
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    if (strcmp(path, pins[i].onCmd) == 0) {
      digitalWrite(pins[i].number, HIGH);
      pins[i].state = 1;
      pins[i].activationTime = millis();
    }
    else if (strcmp(path, pins[i].offCmd) == 0) {
      digitalWrite(pins[i].number, LOW);
      pins[i].state = 0;
    }
  }
...
void handleSettings() {
  String html = "<html><head><meta charset='UTF-8'><title>الإعدادات</title></head><body><h2>إعدادات المخارج</h2><form method='POST' action='/saveSettings'>";
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    html += "<label>" + String(pins[i].name) + ": </label>";
    html += "اسم: <input name='name" + String(i) + "' value='" + String(pins[i].name) + "'> ";
    html += "مؤقت الإيقاف (ms): <input name='timer" + String(i) + "' value='" + String(pins[i].autoOffDuration) + "'><br>";
  }
  html += "<input type='submit' value='حفظ'></form><br><a href='/'>رجوع</a></body></html>";
  server.send(200, "text/html", html);
}
...
void handleSaveSettings() {
  for (int i = 0; i < sizeof(pins)/sizeof(pins[0]); i++) {
    ...
    String timerArg = "timer" + String(i);

    if (server.hasArg(nameArg)) {
      // تحويل النص إلى مصفوفة char
      String pinName = server.arg(nameArg);  // الحصول على النص
      pinName.toCharArray(pins[i].name, sizeof(pins[i].name));  // نسخ النص إلى pins[i].name
    }
    if (server.hasArg(timerArg)) {
      pins[i].autoOffDuration = server.arg(timerArg).toInt();
    }
  }
  savePinStatesToEEPROM();
